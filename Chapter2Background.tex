    ...
    \longsection{PET Image Reconstruction}{sec:pet_image_reconstruction}
        ...
        \subsection{Analytic Image Reconstruction} \label{sec:analytic_image_reconstruction}
            ...
            \begin{figure}
                ...
                \captionsetup{singlelinecheck=false, justification=raggedright}
                \caption{Example of a simulated \glss{AC} \gls{Non-TOF} \gls{BP} reconstruction, with motion and noise, with no randoms or scatters, of the thorax with a spherical lesion in the lungs. Transverse view.}
            ...
            \begin{figure}
                ...
                \captionsetup{singlelinecheck=false, justification=raggedright}
                \caption{Example of a simulated \glss{AC} \gls{Non-TOF} \gls{FBP} reconstruction, with motion and noise, with no randoms or scatters, of the thorax with a spherical lesion in the lungs. Transverse view.}
        ...
        \subsection{Image Registration} \label{sec:image_registration}
            ...
            \subsubsection{Non-Rigid Deformation} \label{sec:non_rigid_deformation}
                \begin{figure}
                    ...
                    \captionsetup{singlelinecheck=false, justification=raggedright}
                    \caption{Graphical representation of a \gls{Non-RD}. On the left a grid can be seen, this grid undergoes a \gls{Non-RD} to the grid on the right.} \label{fig:non_rigid_deformation_nRD}
                \end{figure}
                
                A \gls{Non-RD} is one where the euclidean distance between pairs of points is not maintained, this can be seen in~\Fref{fig:non_rigid_deformation_nRD}. Notice that the euclidean distance between where the lines of the grids intersects changes between the grid on the left and the grid on the right. %For example the motion of a fluid is a \gls{Non-RD} as pairs of points can move past each other by different amounts.
                \glss{Non-RD} are commonly seen in medical imaging in \gls{RM} as the diaphragm and lungs experience sliding motion and displace different parts of the patient's anatomy by different amounts over the respiratory cycle. A \gls{Non-RD} is often represented by a \gls{DVF} which is, in the simplest case, a volume (the same size, with the same number of elements, as the data) where each 'voxel' contains a vector that points either from the voxel in the dynamic image to the same voxel (anatomically, not literally) in the static image or vice versa.
